...
namespace UnityToCustomEngineExporter.Editor.Urho3D.ParticleGraph
{
    ...
    public class ParticleGraphInitUpdateBuilder
    {
        ...
        public ParticleGraphInitUpdateBuilder(Urho3DEngine engine, PrefabContext prefabContext, GraphResource init,
            ...
            if (particleSystem.shape.enabled)
            {
                switch (particleSystem.shape.shapeType)
                {
                    ...
                    case ParticleSystemShapeType.Box:
                        return BuildBox(EmitFrom.Volume);
                    ...
                    case ParticleSystemShapeType.BoxShell:
                        return BuildBox(EmitFrom.Surface);
                        break;
                    case ParticleSystemShapeType.BoxEdge:
                        return BuildBox(EmitFrom.Edge);
                        break;
        ...
        private GraphNode BuildBox(EmitFrom emitFrom)
        {
            var shape = _particleSystem.shape;
            var cone = new Box()
            {
                BoxThickness = shape.boxThickness,
                Rotation = Quaternion.Euler(shape.rotation),
                Translation = shape.position,
                Scale = shape.scale,
                From = emitFrom,
